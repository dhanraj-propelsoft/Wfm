...
<div class="fill header">
  <h4 class="float-left page-title">Print</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$print->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$print->business_id, 'admin']) }}">{{ $print->company_name }}</a></td>
						<td> {{$print->value}} </td>
					<td> {{$print->used}} </td>
				
				<td > <?php $balance=$print->value - $print->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$print->plan_name}}</td><td></td>
				</tr>
				@endforeach
