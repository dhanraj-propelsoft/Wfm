<div class="modal-header">
    <h4 class="modal-title float-right">Edit Log Register</h4>
</div>

    {!! Form::model($log_registers, ['class' => 'form-horizontal validateform']) !!}
    {{ csrf_field() }}
...
<div class="modal-body">
	<div class="form-body">
		...
		<div class="form-group">
			...
			<div class="col-md-12">
				{!! Form::text('log_date', null,['class' => 'form-control accounts-date-picker rearrangedate']) !!}
			</div>
		</div>		
		...
		<div class="form-group">
			...
			<div class="col-md-12">
				<select name="person_type_id" class="form-control select_item">
					...
					<option @if($person_type->id == $log_registers->person_type_id) selected="" @endif value="{{$person_type->id}}"  data-name="{{$person_type->type}}" >{{$person_type->name}}</option>
					@endforeach
				</select>
			</div>
		</div>

		<div class="form-group employee" @if($log_registers->employee_id == null)  style="display:none;" @endif>
		...
		<div class="form-group person" @if($log_registers->person_id == null)  style="display:none;" @endif>
...
$(document).ready(function()
{
	...
	$('.validateform').validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
            url: '{{ route('log_registers.update') }}',
            type: 'post',
            data: {
                _token: '{{ csrf_token() }}',
                _method: 'PATCH',
