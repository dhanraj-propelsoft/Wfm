...
@if(Session::get('module_name') == "accounts")
	@include('includes.accounts')
@elseif(Session::get('module_name') == "inventory")
	@include('includes.inventory')
@elseif(Session::get('module_name') == "trade")
	@include('includes.trade')
@elseif(Session::get('module_name') == "hrm")
...
<div class="fill header"  style="height:43px;width: 102%;background-color: #e3e3e9;margin-left: -10px;">
  	<h5 class="float-left page-title" style="padding-top: 8px;padding-left: 10px;"><b>Discount</b></h5>
	...
	<div class="batch_container">
  	...
  	<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0">
        ...
        @foreach($discounts as $discount)
            <tr>
            	<td width="1" style="padding-left: 7px;">{{ Form::checkbox('discount',$discount->id, null, ['id' => $discount->id, 'class' => 'item_check']) }}<label for="{{$discount->id}}"><span></span></label></td>
                <td>{{ $discount->name }}</td>
				...
				<td>
					...
					@permission('discount-edit')
						<select style="display:none" id="{{ $discount->id }}" class="active_status form-control">
							<option @if($discount->status == 1) selected="selected" @endif value="1">Active</option>
							<option @if($discount->status == 0) selected="selected" @endif value="0">In-Active</option>
						</select>
					@endpermission						
				</td>
                <td>
                	@permission('discount-edit')	
				        <a data-id="{{ $discount->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
					@endpermission

					@permission('discount-delete')
						<a data-id="{{ $discount->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
					@endpermission
				</td>
            </tr>
        @endforeach
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
   ...
   var datatable_options = {"columnDefs": [{"orderable": false,"targets": [0, -1]}],"order": [[1, "asc"]], "stateSave": true ,"lengthMenu": [15, 25, 50, 100]};
	...
	$('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ route('discount.create') }}", function(data) {
	...
	$('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('trade/discount') }}/"+$(this).data('id')+"/edit", function(data) {
        	$('.crud_modal .modal-container').html("");
        	$('.crud_modal .modal-container').html(data);
        });
        $('.crud_modal').modal('show');
	...
	$('body').on('click', '.delete', function(){
		...
		var delete_url = '{{ route('discount.destroy') }}';
