...
<div class="fill header">
  <h4 class="float-left page-title">MemorySize</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				@foreach($memory as $package)
				<tr>
					 <td > {{$package->propel_id}} </td>
					 <td><a href="{{ route('business_profile.show', [$package->business_id, 'admin']) }}">{{ $package->company_name }}</a></td>
					 <td> {{$package->value}} </td>
					<td> {{$package->used}} </td>
					
					<td > <?php $balance=$package->value - $package->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td>
					<td>{{$package->plan_name}}</td><td></td>
				</tr>
				@endforeach
			</tbody>
  </table>
