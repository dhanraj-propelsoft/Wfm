...
<div class="fill header">
  <h4 class="float-left page-title">Departments</h4>
...
<table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($departments as $department)
		<tr>
		   <td width="1">{{ Form::checkbox('department',$department->id, null, ['id' => $department->id, 'class' => 'item_check']) }}<label for="{{$department->id}}"><span></span></label></td>
		  	...
		  	<td>
				...
				@permission('department-edit')
				  <select style="display:none" id="{{ $department->id }}" class="active_status form-control">
					<option @if($department->status == 1) selected="selected" @endif value="1">Active</option>
					<option @if($department->status == 0) selected="selected" @endif value="0">In-Active</option>
				  </select>
				@endpermission
			  </td>
			  <td>
				@permission('department-edit')
			  		<a data-id="{{ $department->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
				@endpermission

				@permission('department-delete')
			 		<a data-id="{{ $department->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
				@endpermission
		  	  </td>
		</tr>
	  @endforeach
