...
<div class="modal-header">
											<h4 class="modal-title float-right">Edit Voucher Format</h4>
										</div>


							{!!Form::model($voucher_formats, ['class' => 'form-horizontal validateform'])!!}
...
<div class="modal-body">
  ...
  {!! Form::hidden('id', null) !!}
    ...
    <div class="form-group"> {{ Form::label('separator_count','Number of separators', ['class' => 'control-label col-md-5 required']) }}
      <div class="col-md-12">	
      {!! Form::selectRange('separator_count', 1, 3, count($account_formats), ['class' => 'form-control select_item' ]) !!}
      </div>
    </div>
    <?php $sep=1;?>
    @foreach($account_formats as $account_format)
    <div class="form-group separator"> 
    
    {{ Form::label('separator','Separator '.$sep,['class' => 'control-label col-md-5']) }}
    
      <div class="col-md-12"> 
      <select name='separator' class='form-control separations select_item'>
	 <option value="">Select Separator</option>
	 @foreach($separators as $separator) 
			<option value="{{$separator->id}}" @if($account_format->separator_id == $separator->id) selected="" @endif data-name="{{$separator->name}}">{{$separator->display_name}}</option>
	 @endforeach
	 </select>
      </div>
    
    
      <div class="form-group preceding" style="@if($account_format->separator_id != 1) display: none; @endif"> {{ Form::label('preceding_zeros','Preceding', ['class' => 'control-label col-md-5']) }}
	      <div class="col-md-12">
	      	{!! Form::selectRange('preceding_zeros', 0, 5, $account_format->value, ['class' => 'form-control select_item']) !!}
	      </div>
      </div>
    ...
    <div class="form-group">
		...
		<div class="col-md-12">
		<?php $format_preview = array(); ?>
	@foreach($account_formats as $account_format)
			<?php
					if($account_format->name == "auto_number") { $format_preview[] = str_pad('1',$account_format->value+1,'0',STR_PAD_LEFT); }
					elseif($account_format->name == "financial_year") { $format_preview[] =  date('Y'); }
					elseif($account_format->name == "voucher_code") { $format_preview[] = 'AA'; }
			?>
	@endforeach	

	<label class='form-control preview'> <?php echo implode("$voucher_formats->icon", $format_preview); ?> </label>
		</div>
      ...
      return str.length < max ? pad("0" + str, max) : str;
		...
		$('.validateform').validate({
            ...
            submitHandler: function(form) {
				...
				$.ajax({
				 url: '{{ route('voucher_format.update') }}',
				 type: 'post',
				 data: {
				 	_token: '{{ csrf_token() }}',
				 	_method: 'PATCH',
