		...
		<div class="row">
			...
			<div class="form-group col-md-6">
				...
				<div class="col-md-12">
	                ...
	                <div class="sale_multi_print">
	                    {{ Form::select('job_estimation_printformate',$multi_print_formates,$sales_templates, ['id' => 'print_id', 'multiple' => 'multiple','class'=>'form-control select_item job_estimation_printformate','style'=>'width: 100%;']) }} 
	               </div>
				</div>
			</div>
		</div>
	...
	$(document).ready(function() {
      ...
      var  voucher_id = $('select[name=voucher_type_id]').val();
              if(voucher_id === "22"){
                  $('.multi_print').show();
                  $('.common_print').hide();
                  $('.sale_multi_print').hide();
              }else if(voucher_id === "21"){
              	$('.sale_multi_print').show();
              	$('.common_print').hide();
              	$('.multi_print').hide();
              }else{
              	$('.common_print').show();
	...
	$('.validateform').validate({
		...
		submitHandler: function(form) {
			...
			data: {
				...
				multi_estimation_id : $('select[name=job_estimation_printformate]').val(),
