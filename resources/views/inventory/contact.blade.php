...
<div class="fill header" style="height:40px;width: 102%;background-color: #e3e3e9;margin-left: -10px;">
	<div style="float: left;margin-right: auto; padding-left: 20px;padding-top: 5px;">

	  <h5 class="float-left page-title"><b>
	  	@if($name == "vendor")
	  		Supplier
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0">
		...
		@foreach($peoples as $customer)
			<tr>
				<td width="1" style="padding-left: 7px;">{{ Form::checkbox('customer',$customer->id, null, ['id' => $customer->id, 'class' => 'item_check']) }}<label for="{{$customer->id}}"><span></span></label></td>
   ...
   var datatable_options = {"columnDefs": [{"orderable": false,"targets": [0, -1]}],"order": [[1, "asc"]], "stateSave": true};
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR)
				{
					...
					if(ledger_limit == true){
						...
						$.get("{{ route('new_customer_data_create',[$name]) }}", function(data) {
						/*	console.log($('.crud_modal .modal-container').length);*/
	...
	$('body').on('click', '.edit', function(e) {
		e.preventDefault();
		  ...
		  $.ajax({
			url: "{{ url('inventory/contact') }}/"+$(this).data('id')+"/edit",
			type: 'get',
			success:function(response, textStatus, jqXHR) {
				...
				{
				$('#add_user').find('#people_type').prop('checked',true);
				$('#add_user').find('#business_type').prop('checked',false);
				}
				else
				{
				$('#add_user').find('#business_type').prop('checked',true);
				$('#add_user').find('#people_type').prop('checked',false);
				}
				...
				$('#add_user').find('select[name=group_name]').val(data.group_id).trigger('change.select2');


				//console.log($('#add_user').find('select[name=group_name]').find('option:selected').val(data.group_id));
  	...
  	$('body').on('click', '.form-update', function(e) {
		...
		$.ajax({
			...
			data: {
				...
				group_id:$('.add_user_modal select[name=group_name]').val(),           
