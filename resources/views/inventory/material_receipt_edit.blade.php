...
{!! Form::model($material_receipt,['class' => 'form-horizontal validateform']) !!}
    {{ csrf_field() }}
...
<div class="form-group">
	...
	<div class="form-group">
		<div class="row">
			...
			<input id="people_type" value="0" type="radio" name="customer" @if($material_receipt->user_type == 0) checked="checked" @endif id="people_type" />
			<label for="people_type"><span></span>People</label>
			<input id="business_type" value="1" type="radio" name="customer" @if($material_receipt->user_type == 1) checked="checked" @endif id="business_type" />
			<label for="business_type"><span></span>Business</label>
			</div>

			<div class="col-md-3 search_container people " style="@if($material_receipt->user_type == 1) display: none; @endif"> 
			...
			<div class="col-md-3 search_container business" style="@if($material_receipt->user_type == 0) display: none; @endif" > {{ Form::label('business', 'Vendor', array('class' => 'control-label required')) }}
			...
			{{ Form::text('date', null, ['class'=>'form-control accounts-date-picker ']) }}
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="row">
			...
			<div class="col-md-3">
				...
				{!! Form::select('store_id', $stores, $material_receipt->store_id, ['class' => 'form-control select_item', 'id' => 'store_id']); !!}
			</div>
			<div class="col-md-3">
				<label class="required" for="rack_id">Rack</label>
	...
	<div class="form-group">
		<div class="row">
			<div class="col-md-3 search_container people" @if($material_receipt->user_type == 1) style="display:none" @endif> 
			...
			<input id="only_for_storage" type="radio" name="usage_type" <?php if ($material_receipt->work_id == null) echo "checked";?> value="0" >
			<label for="only_for_storage"><span></span>Only for Storage</label>
			<input id="received_for_work" type="radio" name="usage_type" <?php if ($material_receipt->work_id != null) echo "checked";?> value="1" >
			<label for="received_for_work"><span></span>Received for work</label>
			</div>

			<div class="col-md-3 type_of_work" @if($material_receipt->work_id == null) style="display:none" @endif>
				<label  for="inventory_category_id">Type of Work</label>
				{!! Form::select('inventory_category_id', [], $material_receipt->work_id, ['class' => 'form-control select_item', 'id' => 'inventory_category_id']); !!}
			</div>
		</div>	
			...
			<tr>
				...
				<td>
					...
					<?php $selected_item = $item->category; ?>
						<option @if($item->id == $receipt_item->item_id) selected="" @endif data-tax="{{$item->include_tax}}" data-purchase_tax="{{$item->include_purchase_tax}}" data-rate = "" value="{{$item->id}}">{{$item->name}}</option>
					@endforeach
						</optgroup>
					</select>
					{{ Form::textarea('description', $receipt_item->description, ['class'=>'form-control', 'style'=>'margin-top:5px; border: 1px dashed #ccc' , 'placeholder' => 'Description', 'size' => '3x2']) }}
				</td>
				<td>
					{{ Form::text('quantity', $receipt_item->quantity, ['class'=>'form-control']) }}
				</td>

				<td>
					<a @if(count($receipt_items) == 1) style="display: none;" @elseif(count($receipt_items) < $total_items) style="display: none;" @endif class="grid_label action-btn delete-icon remove_row"><i class="fa fa-trash-o"></i></a> 

					<a @if(count($receipt_items) != $total_items) style="display: none;" @elseif(count($items) <= count($receipt_items)) style="display: none;" @endif   class="grid_label action-btn edit-icon add_row"><i class="fa fa-plus"></i></a>
...
<!--   </div> -->
	...
	@if($material_receipt->name == "material_receipt")
	<button type="submit" class="btn btn-success tab_send_btn">Send PO </button>
	@endif
...
var current_select_item = null;
	...
	$('.validateform').validate({
		...
		submitHandler: function(form) {
			...
			$.ajax({
			url: '{{ route('material_receipt.update') }}',
			type: 'post',
			data: {
				_token: '{{ csrf_token() }}',
				_method: 'PATCH',
