...
$('input[name=item_name]').on('click', function() {
	...
	var container = $(this).closest('.form-group');
		...
		if((container.find('.item_modal_container')).length > 0) {
			...
			$(this).closest('.form-group').append(`<div style="position: absolute; background: #fff; left: 30px; z-index:2; min-width: 550px; display: block;" class="row  gst_popover item_modal item_modal_container" id="item_modal_container">
				...
				<div class="col-md-12">
					...
					<div class="row">
						<div class="form-group col-md-8">
							{{ Form::label('global_model','Item',array('class' => 'control-label required'))}}
							<div class="ui-widget">
     					...
     					<div  class="form-group col-md-4"> 
							{{ Form::label('global_make', 'Make', array('class' => 'control-label required')) }}
			  		...
			  		<div class="row">
						...
						<div class="form-group col-md-4"> 
					  	...
					  	<div  class="form-group col-md-4"> 
							{{ Form::label('global_type', 'Type', array('class' => 'control-label required')) }}
					...
					<!--<div class="row">
						...
						<div class="row ">
							<div style="display: block;" class="form-group col-md-8"> 
					...
					<div style="display: block; justify-content: initial;" class="modal-footer col-md-12">
						<!-- <a style="color: #007bff; cursor: pointer; display: none;" class="float-left add_global_item">Add New Item</a> -->
				 		...
				 		<button type="button" class="float-right btn btn-success save_item" id="save_item">Save<!--  Entered Item --></button>
				  		<button type="button" class="float-right btn btn-primary reset">Reset</button>
...
global_make.val("").trigger("change");
		...
		$("input[name=global_model]").autocomplete({
		     	...
		     	source: function(request, response) {
		      	...
		      	select: function( event, ui ) {	
		      		...
		      		var search_item = ui.item.label;

		      		var search_model = ui.item.search_model;	      	

		      		<!-- substr( $value->model, 0, strpos( $value->model, "-")) -->		      				

		       		$('input[name=item_name]').val(ui.item.search_model);
		       		$('input[name=global_model]').attr('data-name',ui.item.models);			       		       		    		

		       		//$('input[name=global_model]').val(ui.item.search_model);
		       		...
		       		//to disable all dropdown
		       		$('#global_main_category').attr('disabled',true);
			       	$('#global_category').attr('disabled',true);
			       	$('#global_type').attr('disabled',true);
		...
		//to check duplicate make name

		$('input[name=global_make_name]').on('blur',function(){
			if($('input[name=global_model]').attr('data-name'))
			{
				
				var item_name = $('input[name=global_model]').attr('data-name');
			}
			else
			{
				
				var item_name = $('input[name=global_model]').val();
				
			}
			var make_name = $(this).val();
			var make_box = $('input[name=global_make_name]');
			$.ajax({
			url : '{{ route('check_duplicate_make_name') }}',
			type : 'POST',
			data: {
					_token : '{{ csrf_token() }}',
					make_name : make_name,
			...
			success:function(data)
			{
				...
				if(data.data == true)
				{
					if(make_box.closest('div').find('span').length == 0)
					{
						make_box.closest('div').append('<span class="error" style="color:red;">MAKE name already exists to this item.</span>');
					}
					$('.add_item').prop('disabled',true);
			       	$('#save_item').prop('disabled',true);
			       	$('#global_main_category').attr('disabled',true);
			       	$('#global_category').attr('disabled',true);
			       	$('#global_type').attr('disabled',true);
			       	$('#global_make').attr('disabled',true);
			       	$('#global_make_name').attr('disabled',true);
			       	$('.global_make').show();
				...
				if(data.data == false)
				{
					if(make_box.closest('div').find('span').length > 0)
					{
						make_box.closest('div').find('span').remove();
					}
					$('.add_item').prop('disabled',true);
			       	$('#save_item').prop('disabled',false);
			       	$('#global_main_category').attr('disabled',false);
			       	$('#global_category').attr('disabled',false);
			       	$('#global_type').attr('disabled',false);
			       	$('#global_make').attr('disabled',false);
			       	$('#global_make_name').attr('disabled',false);
			       	$('.global_make').show();
		...
		$('select[name=global_make]').on('change',function(){
			if($('input[name=global_model]').attr('data-name'))
			{
				console.log("data-name"+$('input[name=global_model]').attr('data-name'));
				var item_name = $('input[name=global_model]').attr('data-name');
			}
			else
			{
				console.log("name"+$('input[name=global_model]').val());
				var item_name = $('input[name=global_model]').val();
				
			}
			var make_name = $(this).val();
			var make_box = $('input[name=global_make_name]');
			var modal_box = $('input[name=global_model]');
			...
			{
				//alert();
				var item_get_name = $('input[name=global_model]').val();
					var make_name = data.make_name;
			       	//console.log(make_name);
			
				item_get_name = item_get_name.substring(0, item_get_name.indexOf("-"));
				var item_new_name = item_get_name+'-'+make_name;
				console.log(item_get_name+'-'+make_name);
				if(item_get_name)
				{
					var item_get_name = $('input[name=global_model]').val(item_new_name);
				...
				if(data.data == true)
				{
					...
					if(modal_box.closest('div').find('span').length > 0)
					{
						modal_box.closest('div').find('span').remove();
					}
		...
		$('#global_model').on('blur',function(){
			...
			var make_name = $('select[name=global_make]').val();
			var model_box = $('input[name=global_model]');
			var make_box = $('input[name=global_make_name]');

			$.ajax({
			url : '{{ route('check_duplicate_item_name') }}',
			type : 'POST',
			data: {
					_token : '{{ csrf_token() }}',
					make_name : make_name,
			...
			success:function(data)
			{
				...
				if(model_box.closest('div').find('span').length > 0)
				{
					model_box.closest('div').find('span').remove();
				}
				...
				if(data.data == true)
				{
					if(model_box.closest('div').find('span').length == 0)
					{
						model_box.closest('div').append('<span class="error" style="color:red;">Duplicate. Can not be added or change MAKE to continue.</span>');
					}
					if(make_box.closest('div').find('span').length > 0)
					{
						make_box.closest('div').find('span').remove();
				...
				else if(data.data == false && data.global == true)
				{
					if(model_box.closest('div').find('span').length == 0)
					{
						model_box.closest('div').append('<span class="error" style="color:red;">Item and MAKE exists. If OK, click on ADD to continue ?</span>');
					}
					if(make_box.closest('div').find('span').length > 0)
					{
						make_box.closest('div').find('span').remove();
		...
		$('.reset').on('click',function(){
			//alert();
			$('input[name=global_model]').val('');
			$('select[name=global_make]').val('');
			$('select[name=global_main_category]').val('');
			$('select[name=global_category]').val('');
			$('select[name=global_type]').val('');
			$('input[name=identifier_a]').val('');
		...
		$('body').on('click', '.save_item', function(e) {
        	...
        	else if($('#global_make').val() == "" && $('#global_make_name').val() == '' ) {

        		$('#global_make').closest('div').append('<span class="error" style="color:red">Select Make</span>');

        	}
        	else if($('#global_type').val() == "") {

        		$('#global_type').closest('div').append('<span class="error" style="color:red">Select Type</span>');
        	} 	
			...
			if(($('#global_category') != "" || $.trim($('#global_category')).length > 0 ) && $.trim($('#global_model')).length > 0) {
				...
				if($('input[name=global_model]').attr('data-name'))
				{
					var global_model_name = $('input[name=global_model]').attr('data-name');
				}
				else
				{
					var global_model_name = $('#global_model').val();
				}
	...
	$('input[name=type]').on('change', function() {
		...
		if($('input[name=type]:checked').attr("id") == 'service') {
			$('.main_inventory').hide();
