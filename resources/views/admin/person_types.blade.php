...
<div class="fill header">
  <h4 class="float-left page-title">Person Types</h4>
</div>
...
<table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($person_types as $person_type)
		<tr>
		 <td width="1">{{ Form::checkbox('person_type',$person_type->id, null, ['id' => $person_type->id, 'class' => 'item_check']) }}<label for="{{$person_type->id}}"><span></span></label></td>
		  	<td>{{ $person_type->display_name }}</td>
		  	<td>
		  	@if($person_type->status == '1')
		  		<label class="grid_label badge badge-success status">Active</label>
			@elseif($person_type->status == '0')
		  		<label class="grid_label badge badge-warning status">In-Active</label>
			@endif

			  <select style="display:none" id="{{ $person_type->id }}" class="active_status form-control">
				<option @if($person_type->status == 1) selected="selected" @endif value="1">Active</option>
				<option @if($person_type->status == 0) selected="selected" @endif value="0">In-Active</option>
			  </select>
		  	</td>
		  	<td>
		  		<a data-id="{{ $person_type->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
		  		<a data-id="{{ $person_type->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
		  	</td>
		</tr>
	  @endforeach
	</tbody>
