...
<div class="fill header">
  <h4 class="float-left page-title">Leave Request</h4>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
		...
		<ul class="batch_list">
			@permission('leaves-delete')
				<li><a class="multidelete">Delete</a></li>
			@endpermission
			@permission('leaves-edit')
  ...
  <table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	 @foreach($leaves as $leave)
		<tr>
			@permission('leave-approval')
				<td width="1">{{ Form::checkbox('leave',$leave->id, null, ['id' => $leave->id, 'class' => 'item_check']) }}<label for="{{$leave->id}}"><span></span></label></td>
			@endpermission
		  ...
		  <td>{{$leave->reason}}</td>
		  	...
		  	<td>
				...
				@endif
				@permission('leave-approval')
					<select style="display:none" id="{{ $leave->id }}"  data-id="{{ $leave->employee_id }}" class="active_status form-control">
						<option @if($leave->approval_status == 0) selected="selected" @endif value="0">Pending</option>
						<option @if($leave->approval_status == 1) selected="selected" @endif value="1">Approved</option>
						<option @if($leave->approval_status == 2) selected="selected" @endif value="2">Cancelled</option>
					</select>
				@endpermission
			</td>            
			...
			<td>
				@permission('leaves-edit')
			  		<a data-id="{{ $leave->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
            	@endpermission
            	@permission('leaves-delete')
              		<a data-id="{{ $leave->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
				@endpermission
		  	</td>
		</tr>
	@endforeach
...
$(document).ready(function() {
  ...
  datatable = $('#datatable').DataTable(datatable_options);
	  ...
	  $('body').on('click', '.edit', function(e) {
			e.preventDefault();
			$.get("{{ url('hrm/leaves') }}/"+$(this).data('id')+"/edit", function(data) {
			  $('.crud_modal .modal-container').html("");
			  $('.crud_modal .modal-container').html(data);
			});
			$('.crud_modal').modal('show');
	  ...
	  $('body').on('change', '.active_status', function(e) {
			...
			var id = $(this).attr('id');
			var obj = $(this);
			var url = "{{ route('leaves_status_approval') }}";
