...
<div class="fill header">
  <h4 class="float-left page-title">Exceeded</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
					...
					@foreach($exceeded as $exceeded)
				<tr>
					
						 <td > {{$exceeded->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$exceeded->business_id, 'admin']) }}">{{ $exceeded->company_name }}</a></td>
						 <td> {{$exceeded->addonname}} </td>
						<td> {{$exceeded->value}} </td>
					<td> {{$exceeded->used}} </td>
				<td></td>
