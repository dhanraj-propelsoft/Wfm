...
<div class="fill header">
  <h4 class="float-left page-title">Jobcard</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				@foreach($jobcard as $jobcards)
				<tr>
					
						 <td > {{$jobcards->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$jobcards->business_id, 'admin']) }}">{{ $jobcards->company_name }}</a></td>
						<td> {{$jobcards->value}} </td>
					<td> {{$jobcards->used}} </td>
				
				<td > <?php $balance=$jobcards->value - $jobcards->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td>
				<td>{{$jobcards->plan_name}}</td><td></td>
				</tr>
				@endforeach
