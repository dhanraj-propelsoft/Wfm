<div class="modal-header">
	<h4 class="modal-title float-right">Edit Salary Scale</h4>
</div>

	{!! Form::model($salary_scale,['class' => 'form-horizontal validateform']) !!}
    {{ csrf_field() }}
...
<div class="modal-body">
	<div class="form-body">
			...
			<div class="col-md-6" style="border:1px solid #ccc">
				...
				@foreach($earnings as $earning)
					<div class="row earnings" style="padding: 10px 0;">
						<div class="col-md-5">
							{!! Form::select('earning_id', $salary_earnings, $earning->pay_head_id, ['class' => 'form-control select_item', 'id' => 'earning_id']) !!}
						</div>
						<div class="col-md-4">
							{!! Form::text('earning_value',$earning->value,['class' => 'form-control']) !!}
						</div>

						<div class="col-md-3 action_container">
			...
			<div class="col-md-6" style="border:1px solid #ccc">
				...
				@foreach($deductions as $deduction)
					<div class="row deduction" style="padding: 10px 0;">
						<div class="col-md-5">
							{!! Form::select('deduction_id', $salary_deduction, $deduction->pay_head_id, ['class' => 'form-control select_item', 'id' => 'deduction_id']) !!}
						</div>
						<div class="col-md-4">
							{!! Form::text('deduction_value', $deduction->value,['class' => 'form-control']) !!}
						</div>
		...
		<div class="row">
			...
			<div class="col-md-3">
				<input name="round" type="checkbox" id="round"
				@if($salary_scale->round_off != null) checked="checked" @endif />
				<label class="control-label" for="round"><span></span>Round Off</label>
				
			</div>
		</div>

		<div class="row round_off" @if($salary_scale->round_off ==null) style="display: none;" @endif>
			<div class="col-md-12">
				<div class="form-group">
...
$(document).ready(function()
{
	...
	$("body").on('click', '.remove_column', function() {
		...
		if(attr_name_text.indexOf("earnings")) {

			var class_name = 'earnings';

			$('.'+class_name).find('select[name="'+attr_name+'"] option[value="' + selected_option + '"]').unwrap('<span>');
			if($('.'+class_name).length > 1) {
				...
				$('.'+class_name).find('select[name="'+attr_name+'"] span option').unwrap('<span>');
			}

		}  else if(attr_name_text.indexOf("deduction")) {
		...
		submitHandler: function(form) {
			...
			success:function(data, textStatus, jqXHR) {
				...
				call_back(`<tr role="row" class="odd">
					<td><input id="`+data.data.id+`" class="item_check" name="salary_scale" value="`+data.data.id+`" type="checkbox"><label for="`+data.data.id+`"><span></span></label></td>
