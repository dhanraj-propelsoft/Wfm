...
<div class="fill header">
  <h4 class="float-left page-title">Breaks</h4>
  @permission('work-break-create')
  	...
  	<table id="datatable" class="table data_table" width="100%" cellspacing="0">
		...
		<tbody>
		  @foreach($breaks as $break)
			<tr>
				<td width="1">{{ Form::checkbox('break',$break->id, null, ['id' => $break->id, 'class' => 'item_check']) }}<label for="{{$break->id}}"><span></span></label>
				</td>
			  	<td>{{ $break->name }}</td>
			  	<td>{{ $break->start_time }}</td>
			  	<td>{{ $break->end_time }}</td>
			  	<td>           
				@permission('work-break-edit')	
				  <a data-id="{{ $break->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
				@endpermission
				@permission('work-break-delete')
				  <a data-id="{{ $break->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
				@endpermission
			  </td>
			</tr>
		  @endforeach
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
  ...
  $('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('hrm/breaks') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
