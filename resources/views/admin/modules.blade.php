...
<div class="fill header">
  <h4 class="float-left page-title">Modules</h4>
</div>
...
<table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($modules as $module)
		<tr>
		 <td width="1">{{ Form::checkbox('module',$module->id, null, ['id' => $module->id, 'class' => 'item_check']) }}<label for="{{$module->id}}"><span></span></label></td>
		  	<td>{{ $module->display_name }}</td>
		  	<td>{{ $module->description }}</td>
		  	<td>
		  		<a data-id="{{ $module->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
		  		<a data-id="{{ $module->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
		  	</td>
		</tr>
	  @endforeach
	</tbody>
