...
<div class="modal-header" style="background-color: #e9ecef;">
	<h5 class="modal-title float-right" style="font-weight: bold;"> Add Registered Vehicle</h5>
		             		...
		             		<div class="row custom-panel">
				            	...
				            	<div class="col-md-6 customer_type" style= "@if($customer_type_label == null) display:none @endif"> 
									...
									<div class="" >
										...
										<input id="peoples_type" type="radio" name="customer" value="0" />
										<label for="peoples_type" ><span></span>People</label>
									</div>
				                </div>
...
*/
	...
	$('#peoples_type').on('click', function(){
		$('.people').show();
		$('.business').hide();
	...
	/*$('select[name=registration_number]').on('change', function(event) {
			...
			$.ajax({
				...
				success:function(data, textStatus, jqXHR) {
					...
					if(data.data.user_type == 0) {
						console.log("checked in jc");
						$('#people_type').prop('checked', true);
						console.log($('#people_type').length);
						$('#business_type').prop('checked', true);
						$('.people').show();
						$('.business').hide();
						$('.business select[name=people_id]').prop('disabled', true);
					...
					$("input[name=engine_number]").val(data.data.engine_no);

					//$("#business_id").val(data.data.owner_id);
					//$("#person_id").val(data.data.owner_id);
					$('select[name=people_id]').val(data.data.owner_id);
					$("input[name=chassis_number]").val(data.data.chassis_no);
                    ...
                    $("input[name=last_job_card]").val(data.data.last_update_jc);
                    $("input[name=driver]").val(data.data.driver);
                    $("input[name=driver_contact]").val(data.data.driver_contact);
                    if(data.data.vehicle_permit_type == "1"){
                    	$("input[name=permit_type]").val("Yes");
                    }else{
                    	$("input[name=permit_type]").val("No");
                    }
				    
				    $("input[name=fc_due]").val(data.data.fc_due);
				    $("input[name=permit_due]").val(data.data.permit_due);
				    $("input[name=tax_due]").val(data.data.tax_due);
				    $("input[name=vehicle_insurance]").val(data.data.vehicle_insurance);
				    $("input[name=insurance_due]").val(data.data.vehicle_insurance_due);
				if(data.data.bank_loan == "1"){
					$("input[name=bank_loan]").val("Yes");
				}else{
					$("input[name=bank_loan]").val("No");
				}
				
				$("input[name=month_due_date]").val(data.data.month_due_date);
				$("input[name=warranty_km]").val(data.data.warranty_km);
				$("input[name=warrenty_yrs]").val(data.data.warranty_yrs);
				$("input[name=vehicle_name]").val(data.data.vehicle_name);
	...
	$('.validateform').validate({
		...
		submitHandler: function(form) {
			...
			success:function(data, textStatus, jqXHR) {
					...
					$('select[name=registration_number]').append("<option value='"+data.data.id+"'>"+data.data.registration_no+"</option>");

					$('select[name=registration_number]').val(data.data.id);
