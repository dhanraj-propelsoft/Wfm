...
<div class="fill header">
	<h4 class="float-left page-title">Customer Group Name</h4>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<table id="datatable" class="table data_table" width="100%" cellspacing="0">
		...
		<tbody>
			@foreach($customer_groupings as $customer_groupings)
			<tr>
			<td width="1">{{ Form::checkbox('tank',$customer_groupings->id, null, ['id' => $customer_groupings->id, 'class' => 'item_check']) }}<label for="{{$customer_groupings->id}}"><span></span></label></td>
			<td>{{$customer_groupings->name}}</td>
			<td>{{$customer_groupings->discount_value}}</td>
			<td>{{$customer_groupings->description}}</td>
			<td>@if($customer_groupings->status == 1)
			  		<label class="grid_label badge badge-success status">Approved</label>
				@elseif($customer_groupings->status == 0)
			  		<label class="grid_label badge badge-warning status">Draft</label>
				@endif
				<select style="display:none" id="{{ $customer_groupings->id }}" class="active_status form-control">
					<option @if($customer_groupings->status == 1) selected="selected" @endif value="1">Approved</option>
					<option @if($customer_groupings->status == 0) selected="selected" @endif value="0">Draft</option></td>
		
					<td>          
					<a data-id="{{ $customer_groupings->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
					<a data-id="{{ $customer_groupings->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 	
				</td>
					@endforeach
			</tr>
