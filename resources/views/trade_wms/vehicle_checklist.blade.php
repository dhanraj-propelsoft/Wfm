...
<div class="fill header" style="height:43px;width: 102%;background-color: #e3e3e9;margin-left: -10px;">
	...
	<h5 class="float-left page-title" style="padding-top: 8px;padding-left: 10px;"><b>CheckList</b></h5>
	...
	<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0">
		...
		@foreach($checklists as $checklist)
			...
			<tr>
				...
				<td width="1" style="padding-left: 7px;">{{ Form::checkbox('checklist',$checklist->id, null, ['id' => $checklist->id, 'class' => 'item_check']) }}<label for="{{$checklist->id}}"><span></span></label></td>             

				<td>{{ $checklist->display_name }}</td>              
				...
				<td>
					...
					<!-- @permission('unit-edit') -->
						...
						<select style="display:none" id="{{ $checklist->id }}" class="active_status form-control">

							<option @if($checklist->status == 1) selected="selected" @endif value="1">Active</option>

							<option @if($checklist->status == 0) selected="selected" @endif value="0">In-Active</option>
				...
				@permission('checklist-edit')
						 ...
						 <a data-id="{{$checklist->id}}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a> 

				 @endpermission 

				<!--  @permission('checklist-delete')

                          <a data-id="{{ $checklist->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		datatable = $('#datatable').DataTable(datatable_options);
         ...
         $('.table').find('tr:lt(5) td:last-child').empty();
		...
		$('.add').on('click', function(e) {
			...
			$.get("{{ route('checklist.create') }}", function(data) {
		...
		$('body').on('click', '.edit', function(e) {
			...
			$.get("{{ url('trade_wms/vehicle/checklist') }}/"+$(this).data('id')+"/edit", function(data) {
		...
		$('body').on('change', '.active_status', function(e) {
	  	...
	  	$('body').on('click', '.delete', function(){
			...
			var delete_url = '{{ route('checklist.destroy') }}';
