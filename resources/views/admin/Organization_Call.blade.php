...
<div class="fill header">
  <h4 class="float-left page-title">Call In Hour</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$calls->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$calls->business_id, 'admin']) }}">{{ $calls->company_name }}</a></td>
						<td> {{$calls->value}} </td>
					<td> {{$calls->used}} </td>
				
					<td > <?php $balance=$calls->value - $calls->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$calls->plan_name}}</td><td></td>
				</tr>
				@endforeach
