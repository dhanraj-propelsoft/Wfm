	...
	<div id="page-header" class="{{ Session::get('theme_header') }}">
		...
		@if(Session::get('organization_id'))
	 	@if (App\Custom::check_module_list())
			<div class="dropdown"><a title="Menus" href="#" data-target=".menu" class="smallbutton"><i class="fa icon-arrows-squares"></i></a>
				<div class="drop-menu right float-right menu target">
...
@if(Session::get('organization_id'))
	...
	<?php
		...
		$remaining_data = ['records','sms','employee','customer','supplier','purchase','invoice'];
	...
	@if(isset($transaction_type->name))
	<?php 
		$addon_type = $transaction_type->name;
		$organization_id = Session::get('organization_id');
	?>
	@else
		<?php
	...
	@endif	
		...
		/*@if(!App\Custom::remaining_employee())
			
			$('.employee_add').each(function() {
				$(this).removeClass('add');
			});

			$('body').on('click', '.employee_add', function(e) {
		...
		/*@if(!App\Custom::remaining_voucher($addon_type))

			$('body').on('click', '.voucher_addon', function(e) {
								
				$('.voucher_addon').each(function() {
