...
<div class="fill header">
  <h4 class="float-left page-title">Packages</h4>
</div>
...
<table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($packages as $package)
		<tr>
		 <td width="1">{{ Form::checkbox('package',$package->id, null, ['id' => $package->id, 'class' => 'item_check']) }}<label for="{{$package->id}}"><span></span></label></td>
		  	<td>{{ $package->display_name }}</td>
		  	<td>{{ $package->plan_accounttype }}</td>
		  	<td>
		  	@if($package->status == '1')
			...
			@endif
			  ...
			  <select style="display:none" id="{{ $package->id }}" class="active_status form-control">
				<option @if($package->status == 1) selected="selected" @endif value="1">Active</option>
				<option @if($package->status == 0) selected="selected" @endif value="0">In-Active</option>
			  </select>
		  	</td>
		  	<td>
		  		<a data-id="{{ $package->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
		  		<a data-id="{{ $package->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
		  	</td>
		</tr>
	  @endforeach
	</tbody>
