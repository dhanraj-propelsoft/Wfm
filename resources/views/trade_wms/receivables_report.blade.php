			...
			<div class="col-12">
				<div class="form-inline">  
					<div class="col-md-2 form-group"> 
				 	...
				 	{!! Form::select('customer',$customer,null,['class' =>'form-control select_item customer']) !!}  	 
				  </div>
	                <div class="col-md-2 form-group">
	                    <label class="col-form-label" for="from_date">From Date</label>
	                    {!! Form::text('from_date', null, array('class' => 'form-control date-picker', 'data-date-format' => 'dd-mm-yyyy','style'=>'color: #919191;')) !!}
	                </div>
	                <div class="col-md-2 form-group">
...
<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0" name="mytable">
	<thead>
	  <tr>
		<th class="noExport"> 
	 ...
	 <tbody>
		...
		<tr>
			<td><input id="{{$report->transaction_id}}" class="item_check" name="reports" value="{{$report->transaction_id}}" data-name="{{$report->transaction_id}}" type="checkbox">
			<label for="{{$report->id}}"><span></span></label></td>
			<td>{{$report->voucher_no}}</td>
			<td>{{$report->order_no}}</td>
			<td>{{$report->customer_name}}</td>
			<td>{{$report->date}}</td>
			<td>{{$report->amount}}</td>	
			<td><a data-id="{{$report->id}}" class="grid_label action-btn grid_label edit-icon view"><i class="fa li_eye fa-xs"></i></a></td>		
		</tr>
		 @endforeach
    </tbody>
  </table>
...
<script type="text/javascript">
   var datatable = null;
    ...
    $('.full_modal_content').animate({ height: $(window).height() + 'px' }, 400, function() {
      $.get("{{ url('trade_wms/vouchers') }}/"+id+"/view", function(data) {
