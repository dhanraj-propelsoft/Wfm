<div class="modal-header">
	<h4 class="modal-title float-right">Edit Pay Head</h4>
</div>

	{!! Form::model($payhead_edit,['class' => 'form-horizontal validateform']) !!}
	{{ csrf_field() }}
...
<div class="modal-body">
	<div class="form-body">
		...
		<div class="row">
			...
			<div class="col-md-8 formula" @if($payhead_edit->calculation_type != 1)style="display:none;" @endif>
				<div class="form-group">
		...
		<div class="row parent_pay_head"  @if($payhead_edit->formula != 1)  style="display:none;" @endif>
			<div class="col-md-12 ">
				<div class="form-group">
				{!! Form::label('pay_head_id', 'Parent', array('class' => 'control-label')) !!}
			
				<select name="pay_head_id" class="form-control select_item" multiple>
				@foreach($pay_heads as $pay_head)
					<option @if(in_array($pay_head->id, $selected_pay_heads)) selected='selected' @endif value="{{$pay_head->id}}">{{$pay_head->name}}</option>
				@endforeach
			</select>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-md-12">
				...
				{{ Form::radio('wage_type','0', null, ['class' => 'md-radiobtn', 'id'=> 'hour']) }}
				<label class="control-label" for="hour"><span></span>Hour Based</label>

				{{ Form::radio('wage_type','1', null, ['class' => 'md-radiobtn', 'id'=> 'day']) }}
				<label class="control-label" for="day"><span></span>Day Based</label>

				{{ Form::radio('wage_type','2', null, ['class' => 'md-radiobtn', 'id'=> 'month']) }}
				<label class="control-label" for="month"><span></span>Month Based</label>
				</div>
			</div>
		</div>
		
		<div class="form-group fixed_month" @if($payhead_edit->wage_type != 2)  style="display:none;" @endif>
			{!! Form::label('fixed_month', 'Fixed Month', array('class' => 'control-label col-md-4	 required')) !!}
			<div class="col-md-12">
		...
		<div class="form-group fixed_days" @if($payhead_edit->fixed_month !== 0)  style="display:none;" @endif>
			{!! Form::label('fixed_days', 'Fixed Days', array('class' => 'control-label col-md-4 required')) !!}
			<div class="col-md-12">
				{!! Form::text('fixed_days', null,['class' => 'form-control numbers']) !!}
			</div>
		</div>

		<div class="form-group minimum_attendance" @if($payhead_edit->fixed_month != 1)  style="display:none;" @endif>
			{!! Form::label('minimum_attendance', 'Minimum Attendance (Subtract days from current month)', array('class' => 'control-label col-md-12 required')) !!}
			<div class="col-md-12">
