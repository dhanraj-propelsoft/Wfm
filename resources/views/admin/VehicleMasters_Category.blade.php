...
<div class="fill header">
  <h4 class="float-left page-title">Vehicle Masters Category</h4> 
...
<script type="text/javascript">
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('vehiclemasters_categorycreate') }}", function(data) {
  	...
  	$('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('admin/vehiclemasters_categoryedit') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
...
$('body').on('change', '.active_status', function(e) {
			...
			var id = $(this).attr('id');
			var obj = $(this);
			var url = "{{ route('vehicle_category.status') }}";
