...
import {
  StyleSheet,View ,Platform,Dimensions,PixelRatio,AsyncStorage,FlatList,ActivityIndicator 
} from 'react-native';
import { Container,
  Header,
  Title,
  ...
  Radio,
  Item,
  Picker,Card,CardItem,Form,Input,Grid,Col,
...
export default class VehicleList extends Component {
  ...
  /**
      ...
      LoadDataHandler = async ()=> {
        ...
        const PersonId=await  AsyncStorage.getItem('PersonId', (err, item) => item);
            ...
            let URL_PRO=`${API_URL}/vehicle_list`;
            
            console.log("URL "+URL_PRO);
         
         axios.get(URL_PRO,{ params:{
            ...
            if(response.status==200){
                ...
                this.setState({
                 ...
                 VehicleList: (page==0)?response.data.data:this.state.VehicleList.concat(response.data.data),
       ...
       *  In VehicleList Screen,there are two fields like customer name, job card no.
       *  Search 
   ...
   */
     ...
     // const color_array=['#d8262c','#1a88ff'];
    
   
      //const current_color='#d8262c';
     // current_color=(current_color!='#d8262c')?'#1a88ff':'#d8262c';
      

        return(
                <Card style={[style.card,{borderTopColor:'#ff8008',borderTopWidth:4}]}  >
                    {/* 
                    <CardItem    style={{margin:2,height:0.1}}  >
                        <Icon  name="person"  style={{marginLeft:0,color:'#a1aab8'}}/>
                        <Text note style={[style.mini,{marginLeft:0}]} >{item.customer} </Text>
                        <Icon  name="call"  style={{marginLeft:'auto',color:'#a1aab8'}}/>
                      <Text note style={[style.mini,{marginLeft:0}]} >{item.customer_mobile} </Text>
                     
                    </CardItem> */}
                  
                    <CardItem header bordered first style={{margin:2,height:'auto',borderBottomWidth:0}}  >
                      
                            <View style={{width:'20%',height:'auto'}}>
                                <Button rounded style={{backgroundColor:'#a1aab8'}}>

                                      <Icon name="car"  style={{margin:'auto'}}/>
                                </Button>
                            
                            </View>
                            <View style={{width:'80%',height:'auto'}}>
                            
                                <Text style={[style.mini,{color:'black'}]}  >{item.registration_no} </Text>
                              
                                <Text style={[style.mini]} note>{item.vehicle_configuration} </Text>
                               
                                <Text style={[style.mini]} note>{item.customer} </Text>
                                <Text style={[style.mini]} note>{item.customer_mobile} </Text>
...
//    const isUpdated=navigation.getParam('updated',false);
...
// }
   ...
   */
    ...
    render() {
      ...
      const {  loading,VehicleList,IsSearch,jobcard_no,customer_name,JobcardStatus,Search_status } = this.state;
      ...
      return (
            <Container>
                          ...
                          {/* {this.CustomSearch(this.props.navigation,ObjState,'VehicleList')}
                            */}
