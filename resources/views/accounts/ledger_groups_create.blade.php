...
<div class="modal-header">
  <h4 class="modal-title float-right">Add Ledger Group</h4>
</div>
...
<div class="modal-body">
  <div class="form-body">
	...
	<div class="form-group"> {!! htmlspecialchars_decode(Form::label('parent_id', 'Parent', array('class' => 'control-label col-md-3 required'))) !!}
	  <div class="col-md-12"> {!! Form::select('parent_id', $ledger_groups, null, ['class' => 'select_item form-control', 'id' => 'parent_id']) !!} </div>
	</div>
	...
	<div class="form-group"> {!! htmlspecialchars_decode(Form::label('account_ledger_group_types', 'Ledger Group Type', array('class' => 'control-label col-md-12 required'))) !!}
	  <div class="col-md-12"> @foreach($ledger_types as $ledger_type)
		<input id="group_{{$ledger_type->id}}" type="checkbox" name="account_ledger_group_types[]" value="{{$ledger_type->id}}">
...
$(document).ready(function() {
		...
		$('.validateform').validate({
			...
			submitHandler: function(form) {
				if($('input[name="account_ledger_group_types[]"]:checked').length > 0) {
				$('.loader_wall_onspot').show();
					$.ajax({
