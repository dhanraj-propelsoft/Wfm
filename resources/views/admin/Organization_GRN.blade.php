...
<div class="fill header">
  <h4 class="float-left page-title">GRN</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$grn->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$grn->business_id, 'admin']) }}">{{ $grn->company_name }}</a></td>
						<td> {{$grn->value}} </td>
					<td> {{$grn->used}} </td>
				
					<td > <?php $balance=$grn->value - $grn->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$grn->plan_name}}</td>
					<td></td>
				</tr>
				@endforeach
