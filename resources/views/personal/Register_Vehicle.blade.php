...
<div class="fill header">
	<h4 class="float-left page-title">Registered Vehicles List</h4>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.add').on('click', function(e) {
			e.preventDefault();
			$.get("{{ route('vms.vehicle_registration_create') }}", function(data) {
        ...
        isFirstIteration = true;
			$.get("{{ url('user/vms/myvehicle_edit') }}/"+$(this).data('id')+"/edit", function(data) {
        	$('.crud_modal .modal-container').html("");
        	$('.crud_modal .modal-container').html(data);
        });
			$('.crud_modal').find('.modal-dialog').addClass('modal-lg');
...
$('body').on('change', '.active_status', function(e) {
			...
			var id = $(this).attr('id');
			var obj = $(this);
			var url = "{{ route('vms_activestatus') }}";
