...
<div class="fill header">
  <h4 class="float-left page-title">Service Types</h4>
  ...
  <table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($service_types as $service_type)
		<tr>
			<td width="1">{{ Form::checkbox('holiday',$service_type->id, null, ['id' => $service_type->id, 'class' => 'item_check']) }}<label for="{{$service_type->id}}"><span></span></label></td>	
		  <td>{{ $service_type->display_name }}</td>
		  <td>
		  @if($service_type->is_chargeable == '1')
			  <label class="grid_label badge badge-info status">Chargeable</label>
				@elseif($service_type->is_chargeable == '0')
				  <label class="grid_label badge badge-primary status">Not Chargeable</label>
				@endif
		  </td>
		  <td>@if($service_type->status == '1')
