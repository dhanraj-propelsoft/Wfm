						  	...
						  	<div class="col-md-8"  style="border:2px;border-color: red;"> 
								  ...
								  <!--  @foreach($inventory_types as $type)
								  	...
								  	<div class="col-md-8">
								  		...
								  		<input type="radio" name="type" id="goods" value="1" @if($type_name == 'goods') checked="checked" @endif />
								  		<label for="goods"><span></span>Goods</label>
								  		<input type="radio" name="type" id="service" value="2" @if($type_name == 'service') checked="checked" @endif/>
			...
			submitHandler: function(form) {
				...
				if($('input[name=item_id]').val() != "") {
					...
					data: {
						...
						expense_account: $('select[name=expense_account]:not([disabled])').val(),
						...
						purchase_tax_id:$('select[name=purchase_tax_id]:not([disabled])').val(),
					...
					success:function(data, textStatus, jqXHR) {
							...
							call_back(`<tr>
									...
									<td>
										<img style="border:2px solid #ccc; border-radius: 3px;" src="" width="50" height="50">`+data.data.name+`</td>	
