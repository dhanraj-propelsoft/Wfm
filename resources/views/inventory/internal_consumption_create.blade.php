	...
	<div class="form-group">
	  	<div class="row">
	  	  	<div class="col-md-3">
		  		...
		  		{{ Form::text('consumption_date', ($transaction_type->date_setting == 0) ? date('d-m-Y') : null, ['class'=>'form-control date-picker datetype', 'data-date-format' => 'dd-mm-yyyy', 'autocomplete' => 'off']) }} 
	  		</div>
	  		<div class="col-md-3">
		  		<label class="required" for="department_id">Department</label>
		  		{!! Form::select('department_id', $departments, null,['class' => 'form-control select_item']) !!}
	  		</div>
	  		<div class="col-md-3">
		  		<label class="required"  for="employee_id">Employee</label>
		  		{{ Form::select('employee_id',['' => 'Select Employee'], null, ['class' => 'form-control select_item', 'id' => 'employee_id']) }} 
	  		</div>
	  		<div class="col-md-3">
  	...
  	<div class="form-group">
		<div class="row">
			...
			<div class="col-md-3">
				...
				{!! Form::select('store_id', ['' => 'Select Store'], null, ['class' => 'form-control select_item', 'id' => 'store_id']); !!}
			</div>
			<div class="col-md-3">
  	...
  	<div class="form-group">
		<table style="border-collapse: collapse;" class="table table-bordered crud_table">
			...
			<tr>
				...
				<td>
					<select name="item_id" class="form-control select_item required" id="item_id" >
				...
				<td>{{ Form::text('in_stock', null, ['class'=>'form-control','disabled' => 'true']) }}
				</td>
				<td>{{ Form::text('quantity', null, ['class'=>'form-control decimal', 'disabled' => 'true']) }}
				</td>
  ...
  $(document).ready(function() {
	...
	$( "select[name=department_id]" ).change(function () {
		...
		$('.loader_wall_onspot').show();
			$.ajax({
				 url: '{{ route('get_employee_by_department') }}', // DepartmentController
				 type: 'get',
				 data: {
					department_id: id
					},
	...
	$('body').off('click', '.add_row').on('click', '.add_row', function() {
		...
		if(item.val() != "" && quantity.val() != "" && quantity.val() != 0 ) {
			...
			clone.find('select[name=item_id], input[name=quantity], input[name=in_stock]').val("");
...
$('.validateform').validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
				 url: '{{ route('internal_consumption.store') }}',
				type: 'post',
				data: {
					_token: '{{ csrf_token() }}',					
