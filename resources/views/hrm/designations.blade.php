...
<div class="fill header">
  	<h4 class="float-left page-title">Designations</h4>
	...
	<div class="batch_container">
  	...
  	<table id="datatable" class="table data_table" width="100%" cellspacing="0">
		...
		@foreach($designations as $designation)
			<tr>
				<td width="1">{{ Form::checkbox('designation',$designation->id, null, ['id' => $designation->id, 'class' => 'item_check']) }}<label for="{{$designation->id}}"><span></span></label></td>
				<td>{{ $designation->name }}</td>              
				<td>{{ $designation->department }}</td>              
				<td>{{ $designation->description }}</td>
				<td>{{ $designation->positions }}</td>
				<td>
					...
					@permission('designation-edit')
						<select style="display:none" id="{{ $designation->id }}" class="active_status form-control">
							<option @if($designation->status == 1) selected="selected" @endif value="1">Active</option>
							<option @if($designation->status == 0) selected="selected" @endif value="0">In-Active</option>
						</select>
					@endpermission
				</td>
				<td>           
					@permission('designation-edit')
					  	<a data-id="{{ $designation->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
					@endpermission

					@permission('designation-delete')
					  	<a data-id="{{ $designation->id }}" class="grid_label action-btn delete-icon delete">
...
<script type="text/javascript">
   ...
   var datatable_options = {"columnDefs": [{"orderable": false,"targets": [0, -1]}],"order": [[1, "asc"]], "stateSave": true};
  	...
  	$('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('designations.create') }}", function(data) {
  	...
  	$('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('hrm/designations') }}/"+$(this).data('id')+"/edit", function(data) {
		  	$('.crud_modal .modal-container').html("");
		  	$('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
  	...
  	$('body').on('change', '.active_status', function(e) {
		...
		var id = $(this).attr('id');
		var obj = $(this);
		var url = "{{ route('designations_status_approval') }}";
  	...
  	$('body').on('click', '.delete', function(){
		...
		var delete_url = '{{ route('designations.destroy') }}';
