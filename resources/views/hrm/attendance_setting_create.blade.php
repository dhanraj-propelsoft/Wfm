...
<script>	
	$(document).ready(function() {
		basic_functions();
		$('input[name=standard_working_hours], input[name=min_hours_for_full_day], input[name=min_hours_for_half_day], input[name=min_hours_for_official_half_day], input[name=grace_time]').val("");
	});
	...
	$('.validateform').validate({
		...
		rules: {
		    name: { required: true },               
            /*standard_working_hours: { required: true },
            min_hours_for_full_day: { required: true },
            min_hours_for_half_day: { required: true },                
            min_hours_for_official_half_day: { required: true },                
            grace_time: { required: true },                
            deduction_days: { required: true },                
            cancel_deduction: { required: true },                
            deduct_from: { required: true },*/               
		},

		messages: {
		    name: { required: "Attendance Setting Name is Required." },                
            /*standard_working_hours: { required: "Standard Working Hours is Required." },
            min_hours_for_full_day: { required: "Minimum Hours for Full Day is Required." },
            min_hours_for_half_day: { required: "Minimum Hours for Half Day is Required." },                 
            min_hours_for_official_half_day: { required: "Minimum Hours for Official Half Day is Required." },                 
            grace_time: { required: "Grace Time is Required." },                 
            deduction_days: { required: "Deduction Days is Required." },                 
            cancel_deduction: { required: "Cancel Deduction is Required." },                 
            deduct_from: { required: "Deduct From is Required." },*/                 
	},
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
            url: '{{ route('attendance_setting.store') }}',
            type: 'post',
            data: {
                _token: '{{ csrf_token() }}',
