					...
					<div class="row">
						<div  class="form-group col-md-6"> 
							{{ Form::label('global_type', 'Type', array('class' => 'control-label')) }}
					  		...
					  		{!! Form::select('global_type', $itemtype , null, ['class' => 'select_item form-control', 'id' => 'global_type']) !!}
...
global_make.val("").trigger("change");
		...
		$("input[name=global_model]").autocomplete({
		     	...
		     	source: function(request, response) {
		      	...
		      	select: function( event, ui ) {		      	
		       		...
		       		var tax = $('input[name=item_name]').closest('.modal-container').find('select[name=tax_id]');
		...
		$('body').off().on('click', '.global_make', function(e) {
			e.preventDefault();
		...
		$.ajax({
			url: "{{ route('item.fsm_get_category_type') }}",
			type: 'post',
			data: {
				_token: '{{ csrf_token() }}',
	...
	$('.modal-body').on('click', '.add_gst', function() {
		...
		var tax = $(this).closest('.modal-container').find('select[name=tax_id]');

		if($(this).data('rate') != "") {
			var tax_value = $('.modal-container').find('select[name=tax_id] option[data-value="'+$(this).data('rate')+'"]').val();
