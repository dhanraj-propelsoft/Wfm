...
<div class="fill header" style="height:43px;width: 102%;background-color: #e3e3e9;margin-left: -10px;">
  	...
  	<h5 class="float-left page-title" style="padding-top: 8px;padding-left: 10px;"><b> Job Card On Hold Item Report </b></h5>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td width="1">{{ Form::checkbox('inventory_item[]',$inventory_item->item_id, null, ['id' => $inventory_item->item_id, 'class' => 'check']) }}<label for="{{$inventory_item->item_id}}"><span></span></label>

                </td>
                <td>{{ $inventory_item->order_no }}</td>
                <td>{{ $inventory_item->registration_no }}</td>

                <td>{{ $inventory_item->item }}</td>

                <td><a href="#" class="batch" data-id="{{$inventory_item->item_id}}">{{ $inventory_item->purchase_price }}</a></td>  
                 <td>{{ $inventory_item->batch_number }}</td>                          

                <td>{{$inventory_item->low_stock}}</td>
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/dropzone/dropzone.js') }}"></script> 
  ...
  $('body').on('click', '.batch', function(e){
            e.preventDefault();
            $.get("{{url('inventory/items_batch')}}/"+$(this).data('id')+"/jc_stock",function(data){
