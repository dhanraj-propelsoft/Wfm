...
<div class="fill header">
  <h4 class="float-left page-title">Make</h4>
	...
	<tbody>
				...
				<tr>
					...
					<td>{{$make->id}}</td>
					 <td > {{$make->name}} </td>
					  <td > {{$make->make}} </td>
					<td> {{$make->user_name}} </td>
		
					<td> {{$make->start_date}} </td>
					<td>
                	<a data-id="{{ $make->id}}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
				...
				<td>
					...
					@elseif($make->status == 0)
						<label class="grid_label badge badge-warning status">In-Active
						</label>
						
					@endif

					<select style="display:none" id="{{ $make->id }}" class="active_status form-control">
					<option @if($make->status == 1) selected="selected" @endif value="1">Active</option>
					<option @if($make->status == 0) selected="selected" @endif value="0">In-Active</option>
...
<script type="text/javascript">
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('VehicleMasters_Make_create') }}", function(data) {
  	...
  	$('body').on('click', '.edit', function(e) {
        ...
        isFirstIteration = true;
         $.get("{{ url('admin/vehicle_makeedit') }}/"+$(this).data('id')+"/edit", function(data) {
        	$('.crud_modal .modal-container').html("");
        	$('.crud_modal .modal-container').html(data);
        });
        $('.crud_modal').modal('show');
  ...
  $('body').on('change', '.active_status', function(e) {
			...
			var id = $(this).attr('id');
			var obj = $(this);
			var url = "{{ route('vehicle_make.status') }}";
