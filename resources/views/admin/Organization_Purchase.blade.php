...
<div class="fill header">
  <h4 class="float-left page-title">Purchase</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$Purchases->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$Purchases->business_id, 'admin']) }}">{{ $Purchases->company_name }}</a></td>
						<td> {{$Purchases->value}} </td>
					<td> {{$Purchases->used}} </td>
				
					<td > <?php $balance=$Purchases->value - $Purchases->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$Purchases->plan_name}}</td><td></td>
				</tr>
				@endforeach
