...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$invoices->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$invoices->business_id, 'admin']) }}">{{ $invoices->company_name }}</a></td>
						<td> {{$invoices->value}} </td>
					<td> {{$invoices->used}} </td>
				
					<td > <?php $balance=$invoices->value - $invoices->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
