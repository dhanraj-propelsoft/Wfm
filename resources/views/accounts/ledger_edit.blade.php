	  		...
	  		<div class="form-body">
						...
						<div class="col-md-12"> 
							@foreach($ledger_types as $ledger_type)
			  				<div style="float: left;" class="tooltips col-md-3" data-container="body" data-placement="top" data-original-title="<?php if($ledger_type->name == "personal") { echo 'Ledger such as Staff, Customer, Vendor and Trade agent etc'; } elseif($ledger_type->name == "impersonal") { echo 'Internal accounts such as Income, Expenses, Stock, etc'; } elseif($ledger_type->name == "bank") { echo 'Scheduled, Private and National Bank with IFSC & MICR only'; } elseif($ledger_type->name == "nbfc") { echo 'Finance and loan Labiality accounts other than Scheduled, Private and National Bank'; } ?>">
								<input class="md-radiobtn" id="{{$ledger_type->name}}" type="radio" name="ledger_type" <?php if($ledger_type->id == $ledger->ledger_type) { echo 'checked="checked"'; } ?> value="{{$ledger_type->id}}">
				...
				<div class="row">
		  			<div class="form-group col-md-4"> {!! Form::label('ledger_name', 'Ledger Name', array('class' => 'control-label col-md-6 required')) !!}
						<div class="col-md-12"> {{ Form::text('ledger_name', $ledger->display_name, ['class' => 'form-control display_name']) }}
						</div>
		  			</div>
		  			<div class="form-group col-md-4"> {!! Form::label('ledger_group', 'Parent', array('class' => 'control-label col-md-12 required')) !!}
						<div class="col-md-12"> {!! Form::select('ledger_group', $account_ledger_groups, $ledger->group_id, ['class'=>'form-control select_item']) !!} </div>
		  			</div>
		  			<div class="form-group col-md-4"> {!! Form::label('account_head', 'Account Head', array('class' => 'control-label col-md-12 required')) !!}
						<div class="col-md-12"> {!! Form::text('account_head', $ledger_head, ['class' => 'form-control', 'disabled']) !!} </div>
		  			</div>
				</div>
				<div class="row">
		  			<div class="form-group col-md-4"> {!! Form::label('opening_balance_date', 'Opening Balance on', array('class' => 'control-label col-md-12 required')) !!}
						<div class="col-md-12"> {!! Form::text('opening_balance_date', null, ['class' => 'form-control accounts-date-picker rearrangedate', 'data-date-format' => 'dd-mm-yyyy']) !!} </div>
		  			</div>
		  			<div class="form-group col-md-4"> {!! Form::label('opening_balance','Opening Balance', ['class' => 'col-md-12 control-label']) !!}
				...
				<div @if($ledger->ledger_type_name != "bank") style="display:none" @endif class="row bank">
				  	<div  class="form-group col-md-4"> 
				  		{!! Form::label('bank_name', 'Bank Name', array('class' => 'control-label col-md-6')) !!}
						<div class="col-md-12"> 
							{!! Form::select('bank_name', $bank, null, ['class' => 'select_item form-control','id'=> 'banklist' ]) !!} </div>
				 	</div>
				  	...
				  	<div class="form-group col-md-4"> {!! Form::label('account_no', 'A/c NO', array('class' => 'control-label col-md-4')) !!}
						<div class="col-md-12">{!! Form::text('account_no',$ledger->account_no,['class'=>'form-control']) !!} </div>
				  	</div>
				</div>
				<div @if($ledger->ledger_type_name != "nbfc") style="display:none" @endif class="row nbfc"">
				...
				<div @if($ledger->ledger_type_name != "bank") style="display:none" @endif class="row bank">
		  			<div class="form-group col-md-4">
		  				<?php $selectedstate = isset($selected_list->state) ? $selected_list->state : null; ?>
		  				{!! Form::label('state', 'State', array('class' => 'control-label col-md-6')) !!}
						<div class="col-md-12"> 
							{!! Form::select('state',$states,$selectedstate, ['class' => 'select_item form-control','id'=> 'state_bank','style' => 'display:none','placeholder' => 'Select State']) !!} 
						</div>
		  			</div>
		  			<div class="form-group col-md-4"> 
		  				<?php $selectedcity = isset($selected_list->city) ? $selected_list->city : null; ?>
		  				{!! Form::label('city','City', ['class' => 'col-md-6 control-label']) !!}
						<div class="col-md-12">
							{!! Form::select('city', $cities, $selectedcity, ['class' => 'select_item form-control','id'=> 'city_bank','style' => 'display:none' ]) !!}
						</div>
		  			</div>
		  			<div class="form-group col-md-4"> 
		  				{!! Form::label('bank_branch', 'Bank Branch', array('class' => 'control-label col-md-6')) !!}
						<div class="col-md-12">
							{!! Form::select('bank_branch',$branches,null, ['class' => 'select_item form-control', 'id'=> 'branch','style' => 'display:none']) !!}
						</div>
				  	</div>
				</div>
				<div @if($ledger->ledger_type_name != "bank") style="display:none" @endif class="row bank">
				  	<div class="form-group col-md-4"> {!! Form::label('micr', 'MICR Code', array('class' => 'control-label col-md-6')) !!}
						<div class="col-md-12"> {!! Form::text('micr',$ledger->micr,['class'=>'form-control', 'readonly']) !!} </div>
				  	</div>
				  	<div class="form-group col-md-4"> {!! Form::label('ifsc','IFSC Code', ['class' => 'col-md-6 control-label']) !!}
						<div class="col-md-12">{!! Form::text('ifsc',$ledger->ifsc,['class'=>'form-control', 'readonly']) !!} </div>
				  	</div>
				</div>
	  		</div>
		</div>
...
<div class="save_btn_container">
  	<button type="reset" class="btn btn-default cancel_transaction">Cancel</button>
