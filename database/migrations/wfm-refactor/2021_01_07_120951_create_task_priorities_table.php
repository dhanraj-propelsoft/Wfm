...
class CreateTaskPrioritiesTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::connection('mysql2')->create('task_priorities', function($table){
        $table->increments('id');
        $table->integer('task_id', false);
        $table->integer('priorty_id');
        ...
        $table->foreign('priorty_id')->references('id')->on('mysql2.task_priorities')->onUpdate('cascade')->onDelete('cascade');
    ...
    public function down()
    {
        ...
        Schema::connection('mysql2')->dropIfExists('task_priorities');
    }
}
