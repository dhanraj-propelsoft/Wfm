...
<div class="fill header">
  	<h4 class="float-left page-title">Label</h4>
  	...
  	<table id="datatable" class="table data_table" width="100%" cellspacing="0">
        ...
        @foreach($Labels as $Label)
            <tr>
				  ...
				  <td>{{ GetPharse($Label->phrase) }}</td>
				  <td>{{ $Label->label_name }}</td>
				<td>
					@if($Label->status == 1)
						<label class="grid_label badge badge-success status">Active</label>
					@elseif($Label->status == 0)
						<label class="grid_label badge badge-warning status">In-Active</label>
					@endif

					@permission('discount-edit')
						<select style="display:none" id="{{ $Label->id }}" class="active_status form-control">
							<option @if($Label->status == 1) selected="selected" @endif value="1">Active</option>
							<option @if($Label->status == 0) selected="selected" @endif value="0">In-Active</option>
						</select>
					@endpermission						
				</td>
                <td>
                	@permission('discount-edit')	
				        <a data-id="{{ $Label->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
					@endpermission

					@permission('discount-delete')
						<!-- <a data-id="{{ $Label->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> -->
					@endpermission
				</td>
            </tr>
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
   ...
   var datatable_options = {"columnDefs": [{"orderable": true,"targets": [0]}],"order": [[1, "asc"]], "stateSave": true};
	...
	$('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ route('label.create') }}", function(data) {
	...
	$('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('wfm/label') }}/"+$(this).data('id')+"/edit", function(data) {
        	$('.crud_modal .modal-container').html("");
        	$('.crud_modal .modal-container').html(data);
        });
        $('.crud_modal').modal('show');
	...
	$('body').on('click', '.delete', function(){
		...
		var delete_url = '{{ route('label.destroy') }}';
