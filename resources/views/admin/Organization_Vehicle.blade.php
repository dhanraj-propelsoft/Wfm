...
<div class="fill header">
  <h4 class="float-left page-title">Vehicle</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$vehicles->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$vehicles->business_id, 'admin']) }}">{{ $vehicles->company_name }}</a></td>
						<td> {{$vehicles->value}} </td>
					<td> {{$vehicles->used}} </td>
				
					<td > <?php $balance=$vehicles->value - $vehicles->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td>
					<td>{{$vehicles->plan_name}}</td><td></td>
				</tr>
				@endforeach
