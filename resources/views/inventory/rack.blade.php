...
<div class="fill header">
  ...
  <h4 class="float-left page-title">Rack</h4>
	...
	<tbody>
	  ...
	  @foreach($racks as $rack)
		...
		<tr>
			...
			<td width="1" style="padding-left: 7px;">{{ Form::checkbox('rack',$rack->id, null, ['id' => $rack->id, 'class' => 'item_check']) }}<label for="{{$rack->id}}"><span></span></label></td>

		  <td>{{ $rack->name }}</td>

		  <td>{{ $rack->warehouse_name }}</td>           
		  ...
		  <td>
			...
			@permission('rack-edit')
			  ...
			  <select style="display:none" id="{{ $rack->id }}" class="active_status form-control">

				<option @if($rack->status == 1) selected="selected" @endif value="1">Active</option>

				<option @if($rack->status == 0) selected="selected" @endif value="0">In-Active</option>
		  ...
		  <td>           
			...
			@permission('rack-edit')
			  ...
			  <a data-id="{{ $rack->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
			...
			@permission('rack-delete')
			  ...
			  <a data-id="{{ $rack->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
  ...
  $('.add').on('click', function(e) {
		...
		$.get("{{ route('rack.create') }}", function(data) {
  ...
  $('body').on('click', '.edit', function(e) {
		...
		$.get("{{ url('inventory/rack') }}/"+$(this).data('id')+"/edit", function(data) {
	...
	$(this).parent().find('select').css('display', 'block');
  	...
  	$('body').on('change', '.active_status', function(e) {
			...
			var url = "{{ route('rack_status_approval') }}";
  ...
  $('body').on('click', '.delete', function(){
	...
	var delete_url = '{{ route('rack.destroy') }}';
  ...
  $('body').on('click', '.multidelete', function() {
		...
		var url = "{{ route('rack.multidestroy') }}";
	...
	$('body').on('click', '.multiapprove', function() {
		...
		var url = "{{ route('rack.multiapprove') }}";
