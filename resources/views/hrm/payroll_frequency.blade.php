...
<div class="fill header">
  <h4 class="float-left page-title">Payroll Frequency</h4>
  @permission('payroll-frequency-create')
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	<div class="batch_container">
		...
		<ul class="batch_list">
			@permission('payroll-frequency-delete')
				<li><a class="multidelete">Delete</a></li>
			@endpermission
			@permission('payroll-frequency-edit')
  ...
  <table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($frequencies as $frequency)
		<tr>
			<td width="1">{{ Form::checkbox('frequency',$frequency->id, null, ['id' => $frequency->id, 'class' => 'item_check']) }}<label for="{{$frequency->id}}"><span></span></label></td>
		  <td>{{ $frequency->name }}</td>
		  <td>{{ $frequency->code }} </td>
		  <td>{{ $frequency->frequency_type }}</td>
		  <td>{{ $frequency->day }}</td>
		  <td>@if($frequency->status == '1')
			  <label class="grid_label badge badge-success status">Active</label>
				@elseif($frequency->status == '0')
				  <label class="grid_label badge badge-warning status">In-Active</label>
				@endif

				@permission('payroll-frequency-edit')
				  <select style="display:none" id="{{ $frequency->id }}" class="active_status form-control">
					<option @if($frequency->status == 1) selected="selected" @endif value="1">Active</option>
					<option @if($frequency->status == 0) selected="selected" @endif value="0">In-Active</option>
				  </select>
				@endpermission
			</td>
		  ...
		  <td>
			@permission('payroll-frequency-edit')
			 <a data-id="{{ $frequency->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
			@endpermission
			@permission('payroll-frequency-delete')
			  <a data-id="{{ $frequency->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
			@endpermission
		  </td>
		</tr>
	  @endforeach
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('payroll_frequency.create') }}", function(data) {
  ...
  $('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('hrm/payroll-frequency') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
  ...
  $('body').on('change', '.active_status', function(e) {
	  ...
	  var current = $(this);
	  $.ajax({
	   url: "{{ route('payrollfrequency_status_approval') }}",
	   type: 'get',
	   data: {
		id: id,
		_token :"{{ csrf_token() }}",
  ...
  $('body').on('click', '.delete', function(){
	...
	var delete_url = '{{ route('payroll_frequency.destroy') }}';
	delete_row(id, parent, delete_url);
   });

  $('body').on('click', '.multidelete', function() {
	var url = "{{ route('payroll_frequency.multidestroy') }}";
	multidelete($(this), url);
});

$('body').on('click', '.multiapprove', function() {
	var url = "{{ route('payroll_frequency.multiapprove') }}";
