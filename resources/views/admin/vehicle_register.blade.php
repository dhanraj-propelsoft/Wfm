...
@stop
@if(Session::get('module_name') == "super_admin")
...
<div class="fill header">
	<h4 class="float-left page-title">Registered Vehicles Lists</h4>	
</div>
	...
	<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0">
		<thead>
			<tr>
				...
				<th> Fuel Type </th>
				<th> Vehicle Usage </th> -->
				<th style= "@if($module_name == 'super_admin') display:none @endif"> Description </th>
				<th style= "@if($module_name != 'super_admin') display:none @endif"> Organization Name </th>
				<th style= "@if($module_name != 'super_admin') display:none @endif"> Created On </th>
				<th style= "@if($module_name == 'super_admin') display:none @endif"> Status </th>
				<th> Action </th>
			</tr>
		</thead>
		<tbody>
		@foreach($vehicles_registers as $vehicle)
			<tr>
				...
				<td  style= "@if($module_name == 'super_admin') display:none @endif">{{ $vehicle->description }}</td>
				<td style= "@if($module_name != 'super_admin') display:none @endif">{{$vehicle->org_name}}</td>
				<td style= "@if($module_name != 'super_admin') display:none @endif">{{$vehicle->started_date}}</td>
				<td style= "@if($module_name == 'super_admin') display:none @endif">
					@if($vehicle->status == '1')
