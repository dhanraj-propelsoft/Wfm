<div class="modal-header">
	<h4 class="modal-title float-right">Add Work Allocation</h4>
</div>
	...
	<div class="form-body">
			...
			<div class="row">
				<div class="col-md-4">
					...
					{{ Form::select('make_id', $make, $selected_make, ['class' => 'form-control select_item', 'id' => 'make_id']) }}
					</div>
				</div>
				<div class="col-md-4">
					<div class="form-group">
					<label class="required" for="vehicle_model_id">Model</label>	
					{!! Form::select('vehicle_model_id', $model, null, ['class' => 'select_item form-control','id'=> 'vehicle_model_id' ]) !!}
				</div>
				</div>
				...
				<div class="col-md-4">
					...
					{{ Form::select('work',$work , null, ['class' => 'form-control select_item', 'id' => 'vehicle_model_id']) }}
				</div>
				</div>
			</div>
...
$(document).ready(function()
{
	...
	$('.validateform').validate({
		...
		submitHandler: function(form) {
			...
			$.ajax({
			url: '{{ route('work_allocation.store') }}',
			type: 'post',
			data: {
				_token: '{{ csrf_token() }}',
