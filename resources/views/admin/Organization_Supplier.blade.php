...
<div class="fill header">
  <h4 class="float-left page-title">Supplier</h4>
</div>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
					 ...
					 <td > {{$suppliers->propel_id}} </td>
					  <td><a href="{{ route('business_profile.show', [$suppliers->business_id, 'admin']) }}">{{ $suppliers->company_name }}</a></td>
					<td> {{$suppliers->value}} </td>
					<td> {{$suppliers->used}} </td>
					
					<td > <?php $balance=$suppliers->value - $suppliers->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$suppliers->plan_name}}</td><td></td>
				</tr>
				@endforeach
