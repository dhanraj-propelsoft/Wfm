...
<div class="modal-header">
	<h4 class="modal-title float-right">Add  Variant Version</h4>
</div>
...
<div class="modal-body">
	<div class="form-body">
		...
		<div class="form-group">
			...
			<div class="col-md-12">
				{{Form::hidden('type_id', null, ['class'=>'form-control'])}}
			</div>
		</div>
		<div class="form-group">
  ...
  var colors = ["#919191"];
		...
		$('.variant').change(function(){
			...
			$.ajax({
				...
				url:"{{url('admin/get_make_name')}}/"+ variant ,
				...
				success:function(data){
					...
					$('input[name=type_id]').val(version[i].type_id);
					//$('input[name=type_id]').attr("data-id",version[i].type_id);
					//$('input[name=category_id]').attr("data-id",version[i].category_id);

					$('input[name=category_id]').val(version[i].category_id);

					}*/
					$('input[name=make_id]').val(version.make_name);
					$('input[name=make_id]').attr("data-id",version.vehicle_make_id);
					$('input[name=model_id]').val(version.model_name);
					$('input[name=model_id]').attr("data-id",version.vehicle_model_id);
					$('input[name=type_id]').val(version.type_id);
					//$('input[name=type_id]').attr("data-id",version[i].type_id);
					//$('input[name=category_id]').attr("data-id",version[i].category_id);

					$('input[name=category_id]').val(version.category_id);
					/*var keys=[]
					var values=[]
					i=0
					for (var key in v[0]) 
