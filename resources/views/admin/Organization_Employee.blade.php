...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
					 ...
					 <td > {{$employees->propel_id}} </td>
					  <td><a href="{{ route('business_profile.show', [$employees->business_id, 'admin']) }}">{{ $employees->company_name }}</a></td>
					<td> {{$employees->value}} </td>
					<td> {{$employees->used}} </td>
					
					<td > <?php $balance=$employees->value - $employees->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$employees->plan_name}}</td><td></td>
				</tr>
				@endforeach
