...
<table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($records as $record)
		<tr>
		 <td width="1">{{ Form::checkbox('record',$record->id, null, ['id' => $record->id, 'class' => 'item_check']) }}<label for="{{$record->id}}"><span></span></label></td>
		  	<td>{{ $record->display_name }}</td>
		  	<td>{{ $record->size }}</td>
		  	<td>{{ $record->price }}</td>
		  	<td>{{ $record->plan_accounttype }}</td>
		  	<td>
		  	@if($record->status == '1')
			...
			@endif
			  ...
			  <select style="display:none" id="{{ $record->id }}" class="active_status form-control">
				<option @if($record->status == 1) selected="selected" @endif value="1">Active</option>
				<option @if($record->status == 0) selected="selected" @endif value="0">In-Active</option>
			  </select>
		  	</td>
		  	<td>
		  		<a data-id="{{ $record->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
		  		<a data-id="{{ $record->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
		  	</td>
		</tr>
	  @endforeach
	</tbody>
