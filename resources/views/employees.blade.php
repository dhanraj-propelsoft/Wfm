...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
	 @foreach($employees as $employee)
		<tr>
			<td width="1">{{ Form::checkbox('employee',$employee->id, null, ['id' => $employee->id, 'class' => 'item_check']) }}<label for="{{$employee->id}}"><span></span></label></td>
  ...
  $(document).ready(function() {
	...
	datatable = $('#datatable').DataTable(datatable_options);
	  ...
	  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('staff.create') }}", function(data) {
	  ...
	  $('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('employees') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').find('.modal-dialog').addClass('modal-lg');
