...
<div class="fill header" style="height:40px;width: 102%;background-color: #e3e3e9;margin-left: -10px;">
	...
	<h5 class="float-left page-title" style="padding-top: 8px;padding-left: 10px;"><b>Vehicle Pricing Segment</b></h5>
	...
	<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0">
		...
		@foreach($segments as $segment)
			...
			<tr>
				...
				<td width="1" style="padding-left: 7px;">{{ Form::checkbox('segment',$segment->id, null, ['id' => $segment->id, 'class' => 'item_check']) }}<label for="{{$segment->id}}"><span></span></label></td>             
				...
				<!--  @permission('unit-edit')  -->
						...
						<select style="display:none" id="{{ $segment->id }}" class="active_status form-control">

							<option @if($segment->status == 1) selected="selected" @endif value="1">Active</option>

							<option @if($segment->status == 0) selected="selected" @endif value="0">In-Active</option>
				...
				@permission('segment-edit')
						 ...
						 <a data-id="{{$segment->id}}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a> 

				@endpermission

				@permission('segment-delete')

                          <a data-id="{{ $segment->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.add').on('click', function(e) {
			...
			$.get("{{ route('segment.create') }}", function(data) {
		...
		$('body').on('click', '.edit', function(e) {
			...
			$.get("{{ url('trade_wms/segment') }}/"+$(this).data('id')+"/edit", function(data) {
		...
		$('body').on('change', '.active_status', function(e) {
	  	...
	  	$('body').on('click', '.delete', function(){
			...
			var delete_url = '{{ route('segment.destroy') }}';
