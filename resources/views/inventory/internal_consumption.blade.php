...
<div class="fill header" style="height:43px;width: 102%;background-color: #e3e3e9;margin-left: -10px;">
	...
	<h5 class="float-left page-title" style="padding-top: 8px;padding-left: 10px;"><b>Internal Consumption</b></h5>
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<table id="datatable" class="table data_table table-hover" width="100%" cellspacing="0">
		...
		@foreach($internal_consumptions as $internal_consumption)

			<tr>

				<td width="1" style="padding-left: 7px;">{{ Form::checkbox('internal_consumption',$internal_consumption->id, null, ['id' => $internal_consumption->id, 'class' => 'item_check']) }}<label for="{{$internal_consumption->id}}"><span></span></label></td>   <td>{{ $internal_consumption->first_name }}</td>    

				<td>{{ $internal_consumption->order_no }}</td>              

				<td class="rearrangedatetext">{{ $internal_consumption->date }}</td>
				...
				<td> 
				  ...
				  <a data-id="{{ $internal_consumption->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>	



				

				  <a data-id="{{ $internal_consumption->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a> 
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
  ...
  $('.add').on('click', function(e) {
			...
			$('.full_modal_content').animate({ height: $(window).height() + 'px' }, 400, function() {
					...
					$.get("{{ route('internal_consumption.create') }}", function(data) {
  ...
  $('body').on('click', '.edit', function(e) {
			...
			$('.full_modal_content').animate({ height: $(window).height() + 'px' }, 400, function() {
					...
					$.get("{{ url('inventory/internal-consumption') }}/"+id+"/edit", function(data) {
  ...
  $('body').on('click', '.delete', function(){
	...
	var delete_url = '{{ route('internal_consumption.destroy') }}';
	...
	delete_row(id, parent, delete_url, "{{ csrf_token() }}");
  	...
  	$('body').on('click', '.multidelete', function() {
		...
		var url = "{{ route('internal_consumption.multidestroy') }}";
