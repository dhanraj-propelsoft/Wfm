...
<div class="fill header">
  <h4 class="float-left page-title">Leave Types</h4>
  	@permission('leave-types-create')
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
		...
		<ul class="batch_list">
			@permission('leave-types-delete')
				<li><a class="multidelete">Delete</a></li>
			@endpermission
			@permission('leave-types-edit')
  ...
  <table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	  @foreach($leavetypes as $leavetype)
		<tr>
			<td width="1">{{ Form::checkbox('leavetype',$leavetype->id, null, ['id' => $leavetype->id, 'class' => 'item_check']) }}<label for="{{$leavetype->id}}"><span></span></label></td>
		  	<td>{{ $leavetype->name }}</td>
		  	<td>{{ $leavetype->code }}</td>
		  	<td>@if($leavetype->status == '1')
			  <label class="grid_label badge badge-success status">Active</label>
				@elseif($leavetype->status == '0')
				  <label class="grid_label badge badge-warning status">In-Active</label>
				@endif

				@permission('leave-types-edit')
				  <select style="display:none" id="{{ $leavetype->id }}" class="active_status form-control">
					<option @if($leavetype->status == 1) selected="selected" @endif value="1">Active</option>
					<option @if($leavetype->status == 0) selected="selected" @endif value="0">In-Active</option>
				  </select>
				@endpermission
			</td>
		  	...
		  	<td>
			@permission('leave-types-edit')
			  <a data-id="{{ $leavetype->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
			@endpermission
			@permission('leave-types-delete')
			  <a data-id="{{ $leavetype->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
			@endpermission
		  </td>
		</tr>
	  @endforeach
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/bootstrap-colorpicker/bootstrap-colorpicker.js') }}"></script>
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('leave_types.create') }}", function(data) {
  ...
  $('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('hrm/leave-types') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').find('.modal-dialog').addClass('modal-lg');
  ...
  $('body').on('change', '.active_status', function(e) {
			...
			var id = $(this).attr('id');
			var obj = $(this);
			var url = "{{ route('leavetypes_status_approval') }}";
