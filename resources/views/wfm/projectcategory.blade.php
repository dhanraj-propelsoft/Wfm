...
<div class="fill header">
  	<h4 class="float-left page-title">Project Category</h4>
	@permission('discount-create')
		<a class="btn btn-danger float-right add-project-cat" style="color: #fff">+ New</a>
	@endpermission
</div>
        ...
        @foreach($Projects as $Project)
            <tr>
				...
				<td>
					...
					@permission('discount-edit')
						<select style="display:none" id="{{ $Project->id }}" class="active_status form-control">
							<option @if($Project->status == 1) selected="selected" @endif value="1">Active</option>
							<option @if($Project->status == 0) selected="selected" @endif value="0">In-Active</option>
						</select>
					@endpermission						
				</td>
                <td>
                	@permission('discount-edit')	
				        <a data-id="{{ $Project->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
					@endpermission

					@permission('discount-delete')
						<a data-id="{{ $Project->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
					@endpermission
				</td>
            </tr>
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
   ...
   var datatable_options = {"columnDefs": [{"orderable": true,"targets": [0]}],"order": [[1, "asc"]], "stateSave": true};
	...
	$('.add-project-cat').on('click', function(e) {
        e.preventDefault();
        $.get("{{ route('projectcategory.create') }}", function(data) {
	...
	$('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('wfm/projectcategory') }}/"+$(this).data('id')+"/edit", function(data) {
        	$('.crud_modal .modal-container').html("");
        	$('.crud_modal .modal-container').html(data);
        });
        $('.crud_modal').modal('show');
	...
	$('body').on('click', '.delete', function(){
		...
		var delete_url = '{{ route('projectcategory.destroy') }}';
...
/*
	$('body').on('click', '.multidelete', function() {
  	...
  	$('body').on('change', '.active_status', function(e) {
		...
		var id = $(this).attr('id');
		var obj = $(this);
		var url = "{{ route('projectcategory.status') }}";
