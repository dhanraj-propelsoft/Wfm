...
<div class="fill header">
  <h4 class="float-left page-title">Tank</h4>
...
<table id="datatable" class="table data_table" width="100%" cellspacing="0">
	...
	<tbody>
	 @foreach($tank as $tank)
		...
		<tr>
			<td width="1">{{ Form::checkbox('tank',$tank->id, null, ['id' => $tank->id, 'class' => 'item_check','data-id' => $tank->id]) }}<label for="{{$tank->id}}">
			<span></span></label></td>
	
		<td>{{ $tank->name }}</td>
		<td>{{ $tank->product }}</td>
		<td>{{ $tank->reading_time }}</td>
		<td>{{ $tank->reading_time1 }}</td>
		<td>{{ $tank->reading_time2 }}</td>
		<td>@if($tank->status == 1)
			<label class="grid_label badge badge-success status">Active</label>
			@elseif($tank->status == 0)
			  <label class="grid_label badge badge-warning status">In-Active</label>
			@endif
			<select style="display:none" id="{{ $tank->id }}" class="active_status form-control">
			<option @if($tank->status == 1) selected="selected" @endif value="1">Active</option>
					<option @if($tank->status == 0) selected="selected" @endif value="0">In-Active</option>
					</td>
		</tr>
	  	  @endforeach
	</tbody>
...
<script type="text/javascript">
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('tank.create') }}", function(data) {
  	...
  	$('body').on('click', '.edit', function(e) {
  		...
  		//console.log(item_id);
		e.preventDefault();
		$.get("{{ url('fuel_station/tank_edit') }}/"+item_id, function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
