...
<div class="fill header">
  	<h4 class="float-left page-title">Priority</h4>
        ...
        @foreach($Priorities as $Priority)
            <tr>
				...
				<td>
					...
					@permission('discount-edit')
						<select style="display:none" id="{{ $Priority->id }}" class="active_status form-control">
							<option @if($Priority->status == 1) selected="selected" @endif value="1">Active</option>
							<option @if($Priority->status == 0) selected="selected" @endif value="0">In-Active</option>
						</select>
					@endpermission						
				</td>
                <td>
                	@permission('discount-edit')	
				        <a data-id="{{ $Priority->id }}" class="grid_label action-btn edit-icon edit"><i class="fa li_pen"></i></a>
					@endpermission

					@permission('discount-delete')
						<a data-id="{{ $Priority->id }}" class="grid_label action-btn delete-icon delete"><i class="fa fa-trash-o"></i></a>
					@endpermission
				</td>
            </tr>
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
   ...
   var datatable_options = {"columnDefs": [{"orderable": true,"targets": [0]}],"order": [[1, "asc"]], "stateSave": true};
	...
	$('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ route('priority.create') }}", function(data) {
	...
	$('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('wfm/priority') }}/"+$(this).data('id')+"/edit", function(data) {
        	$('.crud_modal .modal-container').html("");
        	$('.crud_modal .modal-container').html(data);
        });
        $('.crud_modal').modal('show');
	...
	$('body').on('click', '.delete', function(){
		...
		var delete_url = '{{ route('priority.destroy') }}';
