...
<div class="fill header">
  ...
  @permission('person-types-create')
...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	<div class="batch_container">
		...
		<ul class="batch_list">
			@permission('person-types-delete')
				<li><a class="multidelete">Delete</a></li>
			@endpermission
			@permission('person-types-edit')
	...
	<tbody>
	  @foreach($persons as $person)
		<tr>
			<td width="1">
				@if($person->delete_status == 1)
					{{ Form::checkbox('person',$person->id, null, ['id' => $person->id, 'class' => 'item_check']) }}<label for="{{$person->id}}"><span></span></label>
				@endif
			</td>
		  	<td>{{ $person->name }}</td>              
		  	<td>{{ $person->description }}</td>              
		  	<td>
				@if($person->type == '1') Employee 
				@elseif($person->type == '0') Guest
				@endif
		  	</td>
		  	<td>
				@if($person->status == '1')
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('person_types.create') }}", function(data) {
  ...
  $('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('hrm/person-types') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
  ...
  $('body').on('change', '.active_status', function(e) {
			...
			var id = $(this).attr('id');
			var obj = $(this);
			var url = "{{ route('person_type_status_approval') }}";
