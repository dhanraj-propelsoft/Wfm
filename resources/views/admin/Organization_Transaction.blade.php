...
<div class="float-left table_container" style="width: 100%; padding-top: 10px;">
	...
	<tbody>
				...
				<tr>
						 ...
						 <td > {{$transactions->propel_id}} </td>
						 <td><a href="{{ route('business_profile.show', [$transactions->business_id, 'admin']) }}">{{ $transactions->company_name }}</a></td>
						<td> {{$transactions->value}} </td>
					<td> {{$transactions->used}} </td>
				
				<td > <?php $balance=$transactions->value - $transactions->used;
					if ($balance>=0) {
						echo $balance;
						}else{
						echo "0";
					}?> </td><td>{{$transactions->plan_name}}</td><td></td>
				</tr>
				@endforeach
