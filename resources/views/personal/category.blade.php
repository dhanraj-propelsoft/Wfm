...
<div class="fill header">
  <h4 class="float-left page-title">Categories</h4>
...
@if(count($categories) <= 0)
	<div style="text-align: center">You don't have any categories!</div>
@else
...
<script type="text/javascript" src="{{ URL::asset('assets/plugins/data-tables/datatables.min.js') }}"></script>
  ...
  $('.add').on('click', function(e) {
		e.preventDefault();
		$.get("{{ route('personal_category.create') }}", function(data) {
  ...
  $('body').on('click', '.edit', function(e) {
		e.preventDefault();
		$.get("{{ url('user/personal/category') }}/"+$(this).data('id')+"/edit", function(data) {
		  $('.crud_modal .modal-container').html("");
		  $('.crud_modal .modal-container').html(data);
		});
		$('.crud_modal').modal('show');
  ...
  $('body').on('change', '.active_status', function(e) {
      ...
      var id = $(this).attr('id');
      var obj = $(this);
      var url = "{{ route('personal_category_status_approval') }}";
  ...
  $('body').on('click', '.delete', function(){
	...
	var delete_url = '{{ route('personal_category.destroy') }}';
