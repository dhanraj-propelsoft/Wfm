<div class="content">
  <div class="fill header" style="display:none;">
...
<div class="form-body" style="overflow-y: auto; padding: 15px 25px; height: 500px;">
	<div class="form-group">
	  <div class="row">
		<div class="col-md-2 voucher_type" style="display: none;">
		...
		<div style="display: none;" class="col-md-2 date">
		  <label class="required" for="date">Date:</label>
		  {!! Form::text('date', date('dd-mm-yyyy'),['class' => 'form-control accounts-date-picker', 'data-date-format' => 'dd-mm-yyyy']) !!}
		</div>
       ...
       <!-- //it was use for purchase only..so it's hide.. the new field created in next DEV said by Rajesh sir..
		<div style="display: none;"  class="col-md-2 reference_container">
		  ...
		  <input name="transaction_id" type="hidden" value="1"></input> 
	  ...
	  <table class="table crud_table">
		...
		<tbody>
		  <tr>
			...
			<td><input type="textarea" name="description" val="" placeholder="Enter the Description" class="form-control" style="height: 29px;"></td>
...
<script type="text/javascript">
	...
	basic_functions();
		 ...
		 var voucher_type = "{{ $type }}";
		 var voucher_name = "{{ $datas->display_name }}";
		 var voucher_id = "{{ $datas->id }}";
		 //$('.voucher_type').show();	
		 $('.header').show();

		 $('.date').show();

		 $('select[name=voucher_type]').val(voucher_id).trigger('change');
		 $('.voucher_id').text("");
    ...
    $("input[name=order_no]").autocomplete({
		    ...
		    //source: "{{ url('accounts/order_no_search') }}/"+$('input[name=type]').val(),
		    	source: function(request, response) {
	    			$.getJSON("{{ route('transactions_order_no_search') }}", { term: $("input[name=order_no]").val(), type : $('input[name=type]').val() }, 
	              	response);
  				},
				minLength:1,
		      	select: function( event, ui ) {	
		       	 $('input[name=order_no]').val(ui.item.label);
		      	 $('input[name=transaction_id]').val(ui.item.id);
		      	var journal_type = $('input[name=type]').val();
		      	...
		      	if(journal_type == 'receipt')
		      	{
		      		...
		      		$.ajax({
		      		url: '{{ route('get_recepits_id')}}',
		      		type : 'POST',
		      		data:
		      		{
		      			_token : '{{ csrf_token() }}',
		      			type_id : $('input[name=transaction_id]').val(),
		      		},
		      		success:function(data)
		      		{
		      			var id = data.data;
		      			
		      			$('select[name=voucher_type]').val(id);
			...
			success: function(data, textStatus, jqXHR) {
				 ...
				 html+=`<tr>
							  ...
							  <td>`+data.data.order_no+`</td>`;
							   if(data.data.type == "Cash Payment" || data.data.type == "Bank Withdraw" || data.data.type == "Journal Entry"|| data.data.type == "Credit Note"|| data.data.type == "Debit Note")
							   {
        							html+=`<td>`+data.data.ledger_name+`</td>`;

							   }

     					     if(data.data.type == "Cash Receipt" || data.data.type == "Bank Deposit" || data.data.type == "Journal Entry" || data.data.type == "Credit Note"|| data.data.type == "Debit Note" || data.data.type == "WMS Receipt")
     					     {
        						html+=`<td>`+data.data.credit_ledger_name+`</td>`;
